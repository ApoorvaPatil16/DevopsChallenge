...
function managerFunction(domain, cb) {
    if (domain.type != "Real Domain") {
        var mapperObj = {
            ...
            'SequentialDate': generateSequentialDate,
            'SequentialNumber': generateSequentialNumber,
            'SequentialTimeStamp': generateSequentialTimeStamp
        };
        var result = {};
...
function generateTimestamp(obj, cb) {
    ...
    return cb(new Date(mintime.getTime() + Math.random() * (maxtime.getTime() -
        mintime.getTime())));
...
function generateString(obj, cb) {
    ...
    for (var i = 0; i < words; i++) {
        ...
        text = text.substring(0, (randomPosition - 1)) + " " + text.substring(
            randomPosition);
    }
    return cb(text);
}
...
function generateNumber(obj, cb) {
    return cb(Math.floor((Math.random() * (obj.range[0].max - obj.range[0].min)) +
        obj.range[0].min));
}

function generateDecimal(obj, cb) {
    var afterDecimal = Math.floor((Math.random() * (10000 - 1)) + 1);
    var beforeDecimal = Math.floor((Math.random() * (obj.range[0].max - obj.range[
        0].min)) + obj.range[0].min);
        ...
        while (i < obj.transformers.length) {
            ...
            if (obj.transformers[i].name == "CamelCase") {
                ...
                for (var j = 1; j < dataArr.length; j++) {
                    dataArr[j] = dataArr[j].charAt(0).toUpperCase() + dataArr[j].substring(
                        1);
...
function generateSequentialTimeStamp(obj, cb) {
    var date = new Date();
    // date.setDate(date.getDate() + 1);
    return cb(date);
}

function generateSequentialDate(obj, cb) {
    countdate++;
    var date = new Date();
...
function generateSequentialNumber(obj, cb) {
    countnumber++;
    return cb(obj.range[0].min + countnumber);
    // return cb(obj.range[0].max + (obj.range[0].min * countnumber));
}
