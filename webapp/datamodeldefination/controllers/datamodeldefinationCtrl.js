angular.module('datamill')
    .controller('datamodeldefinationController', ['$scope',
        'datamodeldefinationservice', 'listDomainFactory', '$state',
        '$stateParams', '$mdDialog', '$log',
        function($scope, datamodeldefinationservice, listDomainFactory, $state,
            $stateParams, $mdDialog, $log) {
            var domain;
            var domainName;
            ...
            if ($stateParams.mode === 'edit') {
                ...
                if ($stateParams.dataModel.name === $stateParams.datamodelname) {
                    $scope.dataModel = $stateParams.dataModel;
                    updateMaster($scope.dataModel);
                    datamodeldefinationservice.getStructure($stateParams.datamodelname)
                        .then(function(res) {
                            console.log("Here we getting getStructure", res);
                            if (res && res.attributes[0]) $scope.dataModel.attributes =
                                res.attributes;
            ...
            datamodeldefinationservice.getDataModelConfig().then(function(res) {
                $scope.datamodelconf = res;   
            });
            ...
            $scope.showDeliveryOption = function(ev, state) {
                ...
                $mdDialog.show({
                    ...
                    controllerAs: 'ctrl',
                    templateUrl: '/datamodeldefination/' + state + '/templates/' +
                        state + '.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: true,
                    fullscreen: true,
                    locals: {
                        options: $scope.dataModel[state]
                    }
            ...
            $scope.saveDataModel = function() {
                ...
                if ($stateParams.datamodelname) {
                    datamodeldefinationservice.patchDataModel($scope.dataModel,
                        $stateParams.datamodelname).then(function(res) {
                    ...
                    datamodeldefinationservice.postDataModel($scope.dataModel).then(
                        function(res) {
            ...
            $scope.showDialog = function(ev) {
                $mdDialog.show({
                    ...
                    .then(function(result) {
                        ...
                        $scope.dataModel.patternstruct.push({
                            "name": result.name,
                            "attributes": result.structure
                        });
                        $scope.dataModel.patterns.push({
                            "name": result.name,
                            "mix": 0
                        });
